        ...
        try:
            ...
            if search_results:
                for item in search_results:
                    # Edge/fact results expose a .fact attribute
                    if hasattr(item, 'fact') and item.fact:
                        all_facts.add(item.fact)
                    # Node results expose a .summary / .name attribute
                    if hasattr(item, 'summary') and item.summary:
                        all_summaries.add(item.summary)
                    if hasattr(item, 'name') and item.name and item.name != entity_name:
                        all_summaries.add(f"相关实体: {item.name}")
            ...
            logger.info(
                f"Graphiti检索完成: {entity_name}, "
                f"获取 {len(results['facts'])} 条事实, {len(results['node_summaries'])} 个相关节点"
            )
