...
class ZepToolsService:
    ...
    # ------------------------------------------------------------------
    # Internal helpers: direct Cypher access to nodes and edges
    # ------------------------------------------------------------------

    def _fetch_all_nodes_raw(self, graph_id: str) -> List[Dict[str, Any]]:
        """Return raw node dicts from Neo4j for the given group/graph id."""
        async def _query():
    ...
    def _fetch_all_edges_raw(self, graph_id: str) -> List[Dict[str, Any]]:
        ...
        async def _query():
            async with self._graphiti.driver.session() as session:
                r = await session.run(
                    "MATCH (s)-[e:RELATES_TO {group_id: $gid}]->(t) "
    ...
    def search_graph(
        ...
        如果Graphiti search API不可用，则降级为本地关键词匹配。

        Args:
            graph_id: 图谱ID (group_id in Graphiti)
            query: 搜索查询
            limit: 返回结果数量
            scope: 搜索范围，"edges" 或 "nodes"（Graphiti返回EntityEdge；节点模式走本地）
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        raw_nodes = self._fetch_all_nodes_raw(graph_id)

        result: List[NodeInfo] = []
        for rec in raw_nodes:
            props = rec.get('props') or {}
            labels = [l for l in (rec.get('labels') or []) if l != '__Entity__']
            result.append(NodeInfo(
                uuid=rec.get('uuid') or '',
                name=rec.get('name') or '',
                labels=labels,
                summary=rec.get('summary') or props.get('summary', ''),
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        raw_edges = self._fetch_all_edges_raw(graph_id)
        ...
        for rec in raw_edges:
            ...
            edge_info = EdgeInfo(
                uuid=rec.get('uuid') or '',
                name=rec.get('name') or props.get('name', ''),
                fact=rec.get('fact') or props.get('fact', ''),
                source_node_uuid=rec.get('source_uuid') or '',
                target_node_uuid=rec.get('target_uuid') or '',
            )

            if include_temporal:
                def _ts(key: str) -> Optional[str]:
                    val = props.get(key)
                    return str(val) if val is not None else None

                edge_info.created_at = _ts('created_at')
                edge_info.valid_at = _ts('valid_at')
                edge_info.invalid_at = _ts('invalid_at')
                edge_info.expired_at = _ts('expired_at')
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        try:
            rec = self._call_with_retry(
                func=lambda: self._fetch_node_by_uuid_raw(node_uuid),
